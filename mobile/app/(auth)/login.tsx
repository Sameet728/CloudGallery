    ...
    >
      ...
      <View style={styles.hero}>
        <BlurView intensity={78} tint={isDark ? 'dark' : 'light'} style={styles.logoBubble}>
          <Ionicons name="cloud-outline" size={32} color={isDark ? '#fff' : '#000'} />
        </BlurView>
        <Text style={[styles.title, { color: isDark ? '#fff' : '#000' }]}>CloudGallery</Text>
        <Text style={[styles.subtitle, { color: isDark ? '#aaa' : '#595959' }]}>Private library</Text>
      </View>
      ...
      <BlurView intensity={85} tint={isDark ? 'dark' : 'light'} style={[styles.authCard, { width: cardWidth }]}>
        ...
        >
          <BlurView intensity={65} tint={isDark ? 'light' : 'dark'} style={styles.primaryButton}>
            {loading ? <ActivityIndicator color={isDark ? '#000' : '#fff'} /> : <Text style={[styles.primaryText, { color: isDark ? '#000' : '#fff' }]}>Login</Text>}
          </BlurView>
        </TouchableOpacity>

        <TouchableOpacity style={[styles.btnWrapper, { width: controlWidth, marginTop: 12 }]} onPress={() => router.push('/(auth)/register')} activeOpacity={0.84}>
          <BlurView intensity={35} tint={isDark ? 'light' : 'dark'} style={styles.secondaryButton}>
            <Text style={[styles.secondaryText, { color: isDark ? '#000' : '#fff' }]}>Create account</Text>
          </BlurView>
        </TouchableOpacity>
